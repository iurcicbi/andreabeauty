...
export default function ServiziPage() {
  ...
  const categorie = [
    ...
    { value: 'combo', label: 'Combo (Taglio + Barba)' },
    { value: 'colorazione', label: 'Colorazione' },
    { value: 'trattamenti', label: 'Trattamenti' },
    { value: 'altro', label: 'Altro' },
  ];

  const durateComuni = [
    { value: 15, label: '15 minuti' },
    { value: 20, label: '20 minuti (Barba)' },
    { value: 30, label: '30 minuti (Taglio)' },
    { value: 45, label: '45 minuti' },
    { value: 50, label: '50 minuti (Taglio + Barba)' },
    { value: 60, label: '60 minuti (Colorazione)' },
  ...
  const caricaServizi = async () => {
    try {
      setCaricamento(true);
      const risposta = await webservice.get('/api/servizi/cms');
  ...
  const handleSalva = async () => {
    ...
    try {
      ...
      if (servizioCorrente) {
        // Modifica servizio esistente
        await webservice.put(`/api/servizi/${servizioCorrente._id}`, formData);
        setSuccesso('Servizio aggiornato con successo!');
      } else {
        // Crea nuovo servizio
        await webservice.post('/api/servizi', formData);
        setSuccesso('Servizio creato con successo!');
      }
  ...
  const confermaElimina = async () => {
    ...
    try {
      ...
      await webservice.delete(`/api/servizi/${modalElimina.servizio._id}`);
      setSuccesso('Servizio eliminato con successo!');
      caricaServizi();
  ...
  const confermaToggle = async () => {
    ...
    try {
      ...
      await webservice.put(`/api/servizi/${modalToggle.servizio._id}`, {
        attivo: !modalToggle.servizio.attivo,
      });
      
      setSuccesso(`Servizio ${!modalToggle.servizio.attivo ? 'attivato' : 'disattivato'} con successo!`);
      caricaServizi();
  ...
  return (
    <div className="container mx-auto px-2 md:px-4 py-4 md:py-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 md:mb-8">
        <div>
          <h1 className="text-2xl md:text-4xl font-bold">Gestione Servizi</h1>
          <p className="text-sm text-gray-600 mt-1">
            {servizi.length} servizi • {servizi.filter(s => s.attivo).length} attivi
          </p>
        </div>
      ...
      {mostraForm && (
        <Card titolo={servizioCorrente ? 'Modifica Servizio' : 'Nuovo Servizio'} className="mb-4 md:mb-6">
      ...
      {servizi.length === 0 ? (
        <Card>
          <p className="text-center py-8 text-gray-600 text-sm md:text-base">
            Nessun servizio disponibile. Crea il primo servizio!
          </p>
        </Card>
      ) : (
      ...
      <ModalConferma
        ...
        titolo="Elimina Servizio"
        messaggio={`Sei sicuro di voler eliminare il servizio "${modalElimina.servizio?.nome}"? Questa azione non può essere annullata.`}
        tipo="danger"
      ...
      <ModalConferma
        ...
        titolo={modalToggle.servizio?.attivo ? 'Disattiva Servizio' : 'Attiva Servizio'}
        messaggio={
          modalToggle.servizio?.attivo
            ? `Vuoi disattivare il servizio "${modalToggle.servizio?.nome}"? Non sarà più disponibile per le prenotazioni.`
            : `Vuoi riattivare il servizio "${modalToggle.servizio?.nome}"? Tornerà disponibile per le prenotazioni.`
        }
        tipo={modalToggle.servizio?.attivo ? 'warning' : 'success'}
        testoConferma={modalToggle.servizio?.attivo ? 'Disattiva' : 'Attiva'}
        testoAnnulla="Annulla"
