 ...
 * ============================================================================
 * 
 * GET /api/appointments/[id] - Recupera dettagli appuntamento
 * PUT /api/appointments/[id] - Aggiorna appuntamento completo
 * PATCH /api/appointments/[id] - Aggiorna solo stato appuntamento
...
export async function PUT(
  ...
  try {
    ...
    // Verifica permessi: solo lo specialist o admin può modificare completamente
    if (utente.ruolo !== 'specialist' && utente.ruolo !== 'barber' && utente.ruolo !== 'admin') {
      return NextResponse.json(
    ...
    if (data || oraInizio || servizioId) {
      ...
      const sovrapposizioni = await Appuntamento.find({
        ...
        specialista: appuntamento.specialista,
    ...
    await appuntamento.populate({ path: 'specialista', populate: { path: 'utente', select: 'nome cognome' } });
