...
async function processReminderForAppointment(appuntamento, Appuntamento) {
  ...
  try {
    ...
    if (!telefono) {
      console.warn(`⚠️ [REMINDER] Numero mancante per appuntamento ${appuntamento._id}, salto`);
...
async function onIncomingMessage(msg) {
  ...
  try {
    ...
    if (!appuntamento) {
      console.log(`ℹ️ [RISPOSTA] Nessun appuntamento in attesa per ${fromNumber}`);
      return;
    }
...
async function notifySpecialist(appuntamento, type, nomeCliente) {
  try {
    ...
    if (!specialist?.utente?.telefono) {
      console.log('ℹ️ [NOTIFICA] Telefono specialist non disponibile, salto notifica');
      return;
    }
